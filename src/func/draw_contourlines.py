...
def draw_contourlines(Ev, Eh, Eq, E_levels, show_plot=False):
    ...
    for array in cs1.allsegs[0]:
        if np.floor(np.min(array[:])) < y_min:
            y_min = np.floor(np.min(array[:]))
        if np.ceil(np.max(array[:])) > y_max:
            y_max = np.ceil(np.max(array[:]))
        if np.ceil(np.max(array[:])) > x_max:
            x_max = np.ceil(np.max(array[:]))
